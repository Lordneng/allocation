...
export class FullcostDataGridComponent implements OnInit, AfterViewInit {
  ...
  onYearChange(year: any, month: any, version: any, calMarginData?: any) {
    ...
    this.retrieveMasterData().subscribe(res => {
      // console.log("fullcost res :: ", res);
      this.fullcostManualData = res[0];
      this.fullcostData = calMarginData;
      this.retrieveData();
    });
  }

  retrieveMasterData(): Observable<any> {
    // const fullCost = this.calMarginService.getList(this.month, this.year, this.version, conditionHeader.costProductTypeId, conditionHeader.costVersionId, conditionHeader.referencePriceVersionId);
    const fullCostManual = this.calMarginService.getFollCostManual(this.month, this.year, this.version);
    return forkJoin([fullCostManual]);
  }

  retrieveData() {
    console.log("this.fullcostData >> ", this.fullcostData);
    // console.log("this.listMonth >> ", this.listMonth);
    let productData = _.uniqBy(_.cloneDeep(this.fullcostData), 'productName');
    // prepare data month
    this.fullcostList = [];
