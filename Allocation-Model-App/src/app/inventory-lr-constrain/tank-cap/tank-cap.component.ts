...
export class TankCapComponent implements OnInit {
  ...
  ngAfterViewInit(): void {
    ...
    if (this.versionId) {
      this.TankCapService.getVersionById(this.versionId).subscribe((res: any) => {
        console.log("res >> ", res);
        if (res) {
  ...
  async yearChange() {
    ...
    this.TankCapHistory.onYearChange(this.year, this.month, (dataInfo) => {
      ...
      if (this.versionId) {
        this.dataInfo = _.find(this.TankCapHistory.masterData.tankcap, (item) => {
          return item.version === this.version;
        })
        this.TankCapHistory.setDataInfo(this.dataInfo);
      ...
      this.maxVersion = this.TankCapHistory.maxVersion;
