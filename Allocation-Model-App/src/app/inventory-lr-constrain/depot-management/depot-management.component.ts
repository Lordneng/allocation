...
export class DepotManagementComponent implements OnInit {
  ...
  ngAfterViewInit(): void {
    ...
    if (this.versionId) {
      this.depotManagementMeterService.getVersionById(this.versionId).subscribe((res: any) => {
        if (res) {
  ...
  async yearChange() {
    ...
    this.depotManagementHistory.onYearChange(this.year, this.month, dataInfo => {
      ...
      if (this.versionId) {
        this.dataInfo = _.find(this.depotManagementHistory.masterData.depot, (item) => {
          return item.version === this.version;
        })
        this.depotManagementHistory.setDataInfo(this.dataInfo);

      } else {
        this.dataInfo = dataInfo;
        this.version = this.dataInfo.version ? this.dataInfo.version : 0;
      }
      this.maxVersion = this.depotManagementHistory.maxVersion;
      console.log(this.year)
      this.depotManagementMeterDataGrid.onYearChange(this.year, this.month, (this.dataInfo && this.dataInfo.version ? this.dataInfo.version : 0), this.maxVersion, false);
    })
  }
