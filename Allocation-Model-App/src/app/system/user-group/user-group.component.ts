...
export class UserGroupComponent implements OnInit {
  ...
  retrieveMasterData(): Observable<any> {
    ...
    return forkJoin([masterUserGroup]);
  }
  retrieveData() {
  ...
  //     observable.push(this.masterUserGroupService.create(dataInsert));
  //   }
  //   if (dataUpdate.length > 0) {
  //     _.each(dataUpdate, (item) => {
  //       observable.push(this.masterUserGroupService.update(item));
  //     });
  //   }
  //   if (dataRemove.length > 0) {

  //     _.each(dataRemove, (item) => {
  //       observable.push(this.masterUserGroupService.delete(item));
  //     });
  ...
  saveData() {
    ...
    if (!this.dataInfo.isEdit) {
      ...
      const checkDuplicate = _.find(_.cloneDeep(this.dataList), (it) => { return _.toLower(_.trim(it.name)) === _.toLower(_.trim(this.dataInfo.name)) });
      // console.log("checkDuplicate :: ", checkDuplicate);
      // return;
      if (checkDuplicate) {
        Swal.fire({
          title: 'บันทึกไม่สำเร็จ',
          text: 'มี User Group "' + _.trim(this.dataInfo.name) + '" ในระบบแล้ว',
          icon: 'error',
          showConfirmButton: true,
          confirmButtonText: 'ปิด'
        })
