...
{
    public class VolumeConstrainKTService : IVolumeConstrainKTService
    {
        private readonly ILogger<VolumeConstrainKTService> _logger;

        public VolumeConstrainKTService(ILogger<VolumeConstrainKTService> logger)
        {
            _logger = logger;
        }

        private string ErrMessage;
        public ResponseVolumeConstrain ImportVolumeConstrainKT(IFormFile fileInput, [FromServices] IHostingEnvironment hostingEnvironment)
        {
            string fileInputName = fileInput.FileName.Replace(".xlsx", DateTime.Now.ToString("_yyyyMMdd_HHmmsss") + ".xlsx");
            string fileName = $"{hostingEnvironment.ContentRootPath}\\FileUpload\\VolumeKT\\{fileInputName}";
            string returnPath = "FileUpload/VolumeKT/" + fileInputName;
        ...
        {
            var fileName = $"{Directory.GetCurrentDirectory()}{@"\FileUpload\VolumeKT\"}" + fName;
            ...
            try
            {
                if (CheckFileFormat(fileName))
                {
                    ...
                    using (var excelPack = new ExcelPackage())
                    {
                        ...
                        var ws = excelPack.Workbook.Worksheets[3];
        ...
        {
            var fileTemp = $"{Directory.GetCurrentDirectory()}{@"\FileUpload\ExcelTemplate\Template VolumeConstrainKT.xlsx"}";
